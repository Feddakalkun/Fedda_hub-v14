...
try:
    ...
    _NVVFX_IMPORT_ERROR = None
except Exception as exc:
    nvvfx = None
    _NVVFX_AVAILABLE = False
    _NVVFX_IMPORT_ERROR = exc
    warnings.warn(
        f"Nvidia RTX nodes loaded in compatibility mode (nvvfx unavailable: {exc}). "
        "RTXVideoSuperResolution will pass images through unchanged."
    )
...
class RTXVideoSuperResolution(io.ComfyNode):
    ...
    def execute(cls, images: torch.Tensor, resize_type: UpscaleTypedDict, quality: str) -> io.NodeOutput:
        if not _NVVFX_AVAILABLE:
            # Compatibility fallback so workflows stay loadable when nvidia-vfx is unavailable.
